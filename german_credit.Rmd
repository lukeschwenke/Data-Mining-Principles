...
# Update Column Names
colnames(my_df)[1:62] <- names(df)
colnames(my_df)[63] <- "training_r2"
colnames(my_df)[64] <- "holdout_r2"
...
rep_conf <- data.frame(t(sapply(my_df[1:61], function(x) Rmisc::CI(x, ci=0.975)))) %>% na.omit()
# Calculate Width
rep_conf$width <- (conf$upper-conf$lower)*sqrt(0.632)
